...
public class MTCNN {
    ...
    public MTCNN(AssetManager assetManager) throws IOException {
        ...
        pInterpreter = new Interpreter(FaceMatchUtil.loadModelFile(assetManager, MODEL_FILE_PNET), options);
        rInterpreter = new Interpreter(FaceMatchUtil.loadModelFile(assetManager, MODEL_FILE_RNET), options);
        oInterpreter = new Interpreter(FaceMatchUtil.loadModelFile(assetManager, MODEL_FILE_ONET), options);
    }

    /**
     * Face Detection
     * @param bitmap Image to be processed
     * @param minFaceSize The smallest face pixel value. (The greater this value，The faster the detection)
     */
     ...
     * Note: The top line of the CNN input picture, the coordinates are[0..width,0]。So Bitmap needs to fold in half before running the network; the network output is the same.
     *
     * @param bitmap
     * @return
     */
    private Vector<Box> pNet(Bitmap bitmap, int minSize) {
        ...
        while (currentFaceSize <= whMin) {
            ...
            // (1)Image Resize
            Bitmap bm = FaceMatchUtil.bitmapResize(bitmap, scale);
            int w = bm.getWidth();
            int h = bm.getHeight();
            ...
            prob1 = FaceMatchUtil.transposeBatch(prob1);
            conv4_2_BiasAdd = FaceMatchUtil.transposeBatch(conv4_2_BiasAdd);
            ...
            // Face Size Proportional increase
    ...
    private void pNetForward(Bitmap bitmap, float[][][][] prob1, float[][][][] conv4_2_BiasAdd) {
        float[][][] img = FaceMatchUtil.normalizeImage(bitmap);
        float[][][][] pNetIn = new float[1][][][];
        pNetIn[0] = img;
        pNetIn = FaceMatchUtil.transposeBatch(pNetIn);
    ...
    private Vector<Box> rNet(Bitmap bitmap, Vector<Box> boxes) {
        ...
        for (int i = 0; i < num; i++) {
            float[][][] curCrop = FaceMatchUtil.cropAndResize(bitmap, boxes.get(i), 24);
            curCrop = FaceMatchUtil.transposeImage(curCrop);
            rNetIn[i] = curCrop;
        }
    ...
    /**
     * RNETRunning neural network，Write score and bias to boxes
     * @param rNetIn
     * @param boxes
     */
    ...
    private Vector<Box> oNet(Bitmap bitmap, Vector<Box> boxes) {
        ...
        for (int i = 0; i < num; i++) {
            float[][][] curCrop = FaceMatchUtil.cropAndResize(bitmap, boxes.get(i), 48);
            curCrop = FaceMatchUtil.transposeImage(curCrop);
            oNetIn[i] = curCrop;
        }
    ...
    /**
     * ONet Run a neural network and write scores and biases to boxes
     * @param oNetIn
     * @param boxes
     */
