package com.ss2020.project.demorpher.FaceMatch;
...
public class FaceMatchUtil {
    ...
    public static float[][][] normalizeImage(Bitmap bitmap) {
        ...
        for (int i = 0; i < h; i++) { // Note that it is first high and then wide
            for (int j = 0; j < w; j++) {
    ...
    /**
     * Intercept the rectangular box specified in the box (cross-border to be processed), and resize to size * size size, return the data stored in data。
     * @param bitmap
     * @param box
     * @param size
     * return
